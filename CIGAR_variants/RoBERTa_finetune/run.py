...
from torch.nn import CosineSimilarity,LogSoftmax,MSELoss
...
def convert_examples_to_features(name_user, name_official, description_official, action_path, label, tokenizer, args):
    ...
    #source_ids=name_user_ids+name_official_ids+description_official_ids
...
class TextDataset(Dataset):
    ...
    def __init__(self, tokenizer, args, file_path='train', evaluate=False, test=False, pool=None):
        ...
        #list_names_users = [action['names_users'].split(',') for action in list_actions]
...
def train(args, train_dataset, model, tokenizer,pool):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(bar):
            ...
            loss_vec = torch.zeros(len(user_vec),device=args.device, dtype=torch.float)
            
            for i in range(len(user_vec)):
                prob_pos = (CosineSimilarity(dim=0)(user_vec[i],official_vec[i]) + 1) * 0.5 # * tau
                label_pos = torch.tensor([1]).to(args.device)
                label_pos = label_pos.to(torch.float32)
                loss_temp[i][0] = loss_fct(prob_pos,label_pos)
                indice = 1
                ...
                for j in range(len(user_vec)):
                    ...
                    prob_neg = (CosineSimilarity(dim=0)(user_vec[i],official_vec[temp]) + 1) * 0.5 # * tau
                    label_neg = torch.tensor([0]).to(args.device)
                    label_neg = label_neg.to(torch.float32)
                    loss_temp[i][indice] = loss_fct(prob_neg,label_neg)
                ...
                loss_vec[i] = torch.sum(loss_temp[i])
                loss_vec[i] = loss_vec[i] / indice
                    
            # con_loss = -LogSoftmax(dim=1)(loss_temp) # we get the contrastive loss for each user's name in con_loss[x][0]
            # con_loss = torch.sum(con_loss, dim=0)[0] # sum up with dim=0, the sum of contrastive loss for batch is con_loss[0]
            batch_loss = torch.sum(loss_vec)
            batch_loss = batch_loss / len(user_vec)
            ...
            if args.fp16:
                ...
                #logger.info('print loss type:{}'.format(loss.dtype))
                loss.backward()
...
def evaluate(args, model, tokenizer, prefix="",pool=None,eval_when_training=False):
    ...
    for batch in tqdm(eval_dataloader):
        ...
        for i in range(len(user_vec)):
            ...
            for j in range(len(user_vec)):
                ...
                break
                nag_count += 1
                #cos_wrong += [nn.CosineSimilarity(dim=0)(user_vec[i],official_vec[j]).item()]
                #nag_count += 1
                if nag_count == 6:
                    break
    ...
    logger.info("predicted probabilities:{}".format(logits[:5]))
    logger.info("true labels:{}".format(y_trues[:5]))
...
def main():
    ...
    if args.do_train:
        ...
        #logger.info("Now loading the saved model from %s", args.use_saved_model)
        if args.local_rank not in [-1, 0]:
